...
function hostFromContext (context) {
  return process.env.ALLOW_NON_HTTPS
    ? context.req.protocol
    : 'https' + '://' + context.req.get('host')
}
...
function urlFromContext (context) {
  return process.env.ALLOW_NON_HTTPS
    ? context.req.protocol
    : 'https' + '://' + context.req.get('host') + context.req.originalUrl
}
...
function buildModelUrl (protocol, host, apiRoot, modelName, id) {
  ...
  try {
    result = url.format({
      protocol: process.env.ALLOW_NON_HTTPS ? protocol : 'https',
      host: host,
