...
module.exports = {
  ...
  nextUrlFromContext: nextUrlFromContext,
...
/**
 * Returns the fully qualified next request url
 * @public
 * @memberOf {Utils}
 * @param {Object} context
 * @return {String}
 */
function nextUrlFromContext (context) {
  var parts = url.parse(context.req.originalUrl, true);
  if (parts.query['page[size]'] == null) {
    return null;
  }

  parts.query['page[number]'] = (parseInt(parts.query['page[number]'], 10) || 1) + 1;
  delete parts.search;
  return context.req.protocol + '://' + context.req.get('host') +
        decodeURIComponent(url.format(parts));
}
