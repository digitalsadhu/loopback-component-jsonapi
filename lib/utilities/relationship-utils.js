...
function updateHasMany (
  ...
  return RightModel.updateAll({ [leftPKName]: { inq: rightFKValues } }, {
    ...
    .then(models => {
      const idsToUnset = _.difference(
        models.map(m => String(m.id)),
        rightFKValues.map(id => String(id))
      )
...
function updateHasManyThrough (
  ...
  return PivotModel.find({ where: { [leftFKName]: leftPKValue } })
    .then(models => {
      const existingIds = models.map(model => String(model[rightFKName]))
