...
function updateHasMany(
  ...
  return RightModel.updateAll(
    { [leftPKName]: { inq: rightFKValues } },
    {
...
function updateBelongsTo(
  ...
  if (rightPKId === null) {
    return LeftModel.updateAll(
      { [leftPKName]: leftPKValue },
      {
        [leftFKName]: null,
      }
    );
...
function updateHasManyThrough(
  ...
  return PivotModel.find({ where: { [leftFKName]: leftPKValue } }).then(
    (models) => {
...
function linkRelatedModels(relationName, from, to, ctxOptions) {
