...
function getIncludesArray (query) {
  ...
  var includes = relationships.map(function (val) {
    if (val.indexOf('.') === -1) {
      return val.trim()
    }
  ...
  const res = joinEqualIncludes(includes)
  return res
}

function joinEqualIncludes (includes) {
  const joinedIncludes = []
  const store = {}

  includes.forEach(incl => {
    if (typeof incl === 'string') {
      joinedIncludes.push(incl)
      return
    }

    Object.keys(incl).forEach(inclName => {
      if (!store[inclName]) {
        store[inclName] = incl[inclName]
        return
      }

      if (Array.isArray(store[inclName])) {
        store[inclName].push(incl[inclName])
      } else {
        const a = store[inclName]
        store[inclName] = [a, incl[inclName]]
      }
    })
  })
  Object.keys(store).forEach(item => {
    const newItem = Array.isArray(store[item])
      ? joinEqualIncludes(store[item])
      : store[item]
    joinedIncludes.push({ [item]: newItem })
  })
