  ...
  app.remotes().before('**', function (ctx, next) {
    ...
    if (matches.length > 0) {
      ...
      ctx.res.set({'Content-Type': 'application/vnd.api+json'});
			...
			ctx.req.remotingContext.args = ctx.req.remotingContext.args || {};
			ctx.req.remotingContext.args.filter = ctx.req.remotingContext.args.filter || {};
			ctx.req.remotingContext.args.filter.include = include;
