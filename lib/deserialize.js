...
module.exports = function (app, options) {
  ...
  app.remotes().before('**', function (ctx, next) {
    ...
    if (RelUtils.isJSONAPIPagination(ctx.req)) {
      ctx.res.set({'Content-Type': 'application/vnd.api+json'});

      var pagination = RelUtils.getPagination(ctx.req.query);
      ctx.args = ctx.args || {};
      ctx.args.filter = ctx.args.filter || {};
      ctx.args.filter.limit = pagination.limit;
      ctx.args.filter.skip = pagination.skip;
    }

    if (ctx.req.query.hasOwnProperty('filter')) {
