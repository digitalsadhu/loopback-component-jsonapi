...
/**
 * Handles a single includes request.
 * @private
 * @memberOf {Serializer}
 * @param {Object} resp
 * @param {String}
 * @return {undefined}
 */
function handleSingleInclude(resp, include, relations) {
	if (!relations[include]) {
		throw RelUtils.getInvalidIncludesError('Can\'t locate relationship "' + include + '" to include');
	}

	var iterator = _.isArray(resp.data) ? resp.data : [resp.data];
	var relationships = [];
	...
	iterator.forEach(function (resource) {
		var rel;
		var ids = [];
		var compoundIncludes;

		if (resource.relationships[include] && resource.attributes[include]) {
			if (_.isArray(resource.attributes[include])) {
				while (resource.attributes[include].length) {
					rel = utils.clone(resource.attributes[include].shift());
					ids.push(rel[propertyKey]);
					compoundIncludes = createCompoundIncludes(rel, propertyKey, include);
					relationships.push(compoundIncludes);
				}

				// Reassign it to just the ids
				resource.attributes[include] = ids;

			} else {
				rel = utils.clone(resource.attributes[include]);
				compoundIncludes = createCompoundIncludes(rel, propertyKey, include);
				resource.attributes[include] = resource.attributes[include][propertyKey];
	...
	if (relationships.length) {
		resp.included = relationships;
	}
}

/**
 * Handles an array of includes request.
 * @private
 * @memberOf {Serializer}
 * @param {Object} resp
 ...
 * @return {undefined}
 */
function handleMultipleIncludes(resp, includes, relations) {
	var relationships = [];
	var iterator = _.isArray(resp.data) ? resp.data : [resp.data];
