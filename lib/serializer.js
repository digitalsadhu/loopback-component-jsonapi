      ...
      if (isDeepIncludesExists) {
        includes.forEach(incl => {
          if (typeof incl === 'object') {
            const prop = Object.keys(incl)[0]
            const nestedProp = incl[prop]
            included = getProps(included, prop, nestedProp)
          }
        })
      }
    } else {
      const prop = Object.keys(includes)[0]
      const nestedProp = includes[prop]
      included = getProps(unique, prop, nestedProp)
    }
...
function getProps (included, prop, nestedName) {
  if (typeof nestedName === 'string') {
    return getNestedIncludes(included, prop, nestedName)
  }

  if (Array.isArray(nestedName)) {
    let newInclude = included
    nestedName.forEach(name => {
      newInclude = getProps(newInclude, prop, name)
    })
    return newInclude
  }

  const nestedProp = Object.keys(nestedName)[0]
  let newInclude = included
  newInclude = getNestedIncludes(newInclude, prop, nestedProp)
  return getProps(newInclude, nestedProp, nestedName[nestedProp])
}

function getNestedIncludes (included, prop, nestedProp) {
    ...
    (acc, item) => {
      if (
        ...
        if (_.has(item.attributes, nestedProp)) {
          if (Array.isArray(item.attributes[nestedProp])) {
            nestedItems = item.attributes[nestedProp].map(nestedItem => {
              nestedItem.type = nestedProp
              return createNewIncludeItem(nestedItem)
            })
          } else {
            item.attributes[nestedProp].type = nestedProp
            nestedItems = createNewIncludeItem(item.attributes[nestedProp])
          }
        ...
        if (!item.relationships) {
          item.relationships = { [nestedProp]: {} }
        }

        if (!item.relationships[nestedProp]) {
          item.relationships[nestedProp] = {}
        }
        ...
        if (Array.isArray(nestedItems)) {
          newRelation = nestedItems.map(nestedItem => {
            return { id: nestedItem.id, type: nestedItem.type }
          })
          newRelation = _.uniqWith(
            newRelation,
            (a, b) => a.id === b.id && a.type === b.type
          )
        } else {
          newRelation = { id: nestedItems.id, type: nestedItems.type }
        }

        item.relationships[nestedProp].data = newRelation
