...
describe('loopback json api errors', function () {
  ...
  describe('headers', function () {
    it("GET /models/100 (which doens't exist) should return JSON API header", function (done) {
      request(app).get('/posts/100')
        .expect('Content-Type', 'application/vnd.api+json; charset=utf-8')
        .end(done)
    })

    it('POST /models should return JSON API header when type key not present error is thrown', function (done) {
  ...
  describe('status codes', function () {
    ...
    it('POST /models should return an 422 error on the error object if model title is not present', function (done) {
      Post.validatesPresenceOf('title')
